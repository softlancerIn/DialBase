...
@section('content')
    ...
    <section class="pt-5 pb-5 bg-light">
        <div class="container">
            <div class="row mb-4">
                <div class="col-12">
                    <h2 class="mb-0">{{ $seoData->page_title ?? 'Blogs' }}</h2>
                        ...
                        <div class="card h-100">
                            @if (!empty($blog->image))
                                <a href="{{ route('web_blog_details', $blog->slug) }}">
                                    <img class="card-img-top" src="{{ asset('upload_image/blog/' . $blog->image) }}"
                                        alt="{{ $blog->name }}">
                                </a>
                            @endif
                            <div class="card-body">
                                ...
                                @else
                                    <p class="card-text">
                                        {{ \Illuminate\Support\Str::limit(strip_tags($blog->description), 120) }}</p>
                                @endif
                            </div>
                            <div class="card-footer d-flex justify-content-between align-items-center">
                                <small class="text-muted">{{ optional($blog->created_at)->format('M d, Y') }}</small>
                                <a class="btn btn-sm btn-outline-primary"
                                    href="{{ route('web_blog_details', $blog->slug) }}">Read More</a>
                            </div>
                        </div>
                    </div>
                @empty
